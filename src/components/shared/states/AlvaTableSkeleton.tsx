  ...
  return (
    <div className={cn("space-y-3", className)}>
      <div className="hidden overflow-hidden rounded-2xl bg-alva-surface md:block">
        <div className="grid gap-3 border-b border-alva-border px-4 py-3" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
        ...
        {Array.from({ length: rows }).map((_, rowIndex) => (
          <div
            key={`row-${rowIndex}`}
            className="grid gap-3 border-b border-alva-border/60 px-4 py-3 last:border-0"
      ...
      <div className="space-y-2 md:hidden">
        {Array.from({ length: Math.min(rows, 4) }).map((_, index) => (
          <div key={`mobile-${index}`} className="rounded-2xl bg-alva-surface px-3 py-3">
